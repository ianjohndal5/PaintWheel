...
namespace PaintWheel.Common.Configs.CustomUI;
...
public class PaintPaletteElement : ConfigElement<List<ItemDefinition>>
{
	...
	private void Toggle(int itemType)
	{
		...
		// Every copy goes, as in the in-game editor, so one click always takes a colour out of the ring.
		if (list.RemoveAll(definition => definition is { IsUnloaded: false } && definition.Type == itemType) == 0)
	...
	protected override void DrawSelf(SpriteBatch spriteBatch)
	{
		...
		for (int i = 0; i < offered.Count; i++) {
			int type = offered[i];
			int order = PaintPreset.RingOrder(chosen, type);
			...
			Color fill = order > 0 ? accent : Color.Lerp(accent, UIColors.CellRim, 0.25f);

			WheelDrawing.DrawPaletteCell(spriteBatch, CellRect(dimensions, i), fill, order > 0, i == hovered,
				order > 0 ? order.ToString() : null, 1f);
		}
	}
