...
- [Keybinds](#keybinds)
- [Palettes and presets](#palettes-and-presets)
...
- **A paint you have run out of keeps its place** in a preset, dimmed and struck through, so a saved
  palette never comes up blank and the layout never shifts under your hand mid-build.
...
- **An eyedropper** that copies the paint off any tile or wall — or off a paint in your inventory or a
  chest, by hovering it and pressing the key, even while you are carrying a stack on the cursor. A dot
  of that colour sits beside the cursor while you hover, gold once it is the one you have selected.
...
| --- | --- | --- |
| Open Paint Wheel | `V` | Opens the picker. An alternative to right click. |
| Paint Eyedropper | Middle mouse | Copies the paint off the tile, wall or item under the cursor. |
...
Click the **centre of the wheel** to open your palette list.

- There is always one auto-filled palette — **"Everything you own"** — holding every paint you are
  currently carrying.
- Below it sits one entry per preset you have saved.
- Click the one you want. The choice is remembered per character.

Presets are built from a single palette block in the config: every paint the game has, laid out for
you to click and add or remove. No item pickers full of swords. You can also build one in-game from
what you are carrying, then edit or rename it later.
...
holding. Leaving the mode puts both back. This is the one and only thing in the mod that moves an
item.
...
| Presets | — | Your saved palettes. |

### Sizes and cosmetics

Show item sprites and a background panel (both off by default), plus the geometry: wheel radius,
...
empty swatch stays in place, dimmed and struck through. By default painting stops rather than
spending a different colour; *Advanced → Use any paint when the choice runs out* changes that.

**Blocks I place are not painted.** That is the Paint Sprayer's job. Equip one, or turn on
*Advanced → Paint blocks as you place them*.
...
Clone into your `Terraria/tModLoader/ModSources` folder and build from the in-game **Mod Sources**
menu, or open `PaintWheel.csproj` in your IDE.
